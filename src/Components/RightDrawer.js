  ...
  const list = (anchor) => (
    ...
    >
      <List sx={props.customTop}>
        <div class={styles.rowDiv} style={{justifyContent: 'space-between'}}><div style={{display: 'flex'}}><AccountCircle></AccountCircle><Typography component="div" color='black' variant='h6'>My wallet</Typography></div>
        {isLogined ? <span>{`${isLogined.slice(0,6)}...${isLogined.slice(-4)}`}</span> : <></>}   {/* // 로그인 이후 주소 띄우는 곳 */}
        </div>
        <Divider />
        ...
        {isLogined ? <></>: <ConnectWallet isLogined={isLogined} />}   {/* // 로그인 이후 (total balance 보여주기) : 로그인 이전 (연결 지갑 보여주기) */}
